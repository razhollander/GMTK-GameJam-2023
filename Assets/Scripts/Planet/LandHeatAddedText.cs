...
public class LandHeatAddedText : MonoBehaviour
{
    ...
    public async UniTask Play(float heatAmount, bool isFullOpacity)
    {
        ...
        textTransform.localScale = Vector3.one * (_minimumScale + _scaleStep * Math.Abs(heatAmount));
        ...
        textTransform.localPosition = Vector3.zero;// screenUpVectorInWorldSpace * _startingYOffset;
        textTransform.DOMove(textTransform.position + screenUpVectorInWorldSpace*_addedYOffset, _animationTime).OnComplete(OnCompleteEffect).SetEase(Ease.OutCubic);
        _text.DOFade(0, _animationTime).SetEase(Ease.InQuart);
        textTransform.localScale = Vector3.one * _scaleFactor;
        textTransform.DOScale(1, _animationTime).SetEase(Ease.OutCubic);
    }
